...
if __name__ == "__main__":
    ...
    all_files = list(folder_path_songs.glob("**/*.ogg"))
    print(len(all_files))

    with open('real_definite_paths.txt', 'r') as f:
        file_names = [pathlib.Path(line.strip()) for line in f]
    ...
    for file_name in tqdm(file_names):
        ...
        if os.path.exists(output_path):
            pass#print("embedding already exists")

        else:
            if not str(file_name) == "data/audio/music_tags/electronic-neither-expressive-neither-neither-technological-neither-modern-neither-youthful/IT0311500117.ogg":
