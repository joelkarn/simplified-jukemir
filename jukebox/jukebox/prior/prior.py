...
class SimplePrior(nn.Module):
    ...
    def prior_preprocess(self, xs, conds):
        ...
        for i in range(len(xs)):
            ...
            assert isinstance(x, t.LongTensor.to("mps")), x
        ...
        for i in range(len(conds)):
            ...
            if cond is not None:
                ...
                conds[i] = t.zeros((N, dims, self.prior_width), dtype=t.float, device='mps')
    ...
    def get_prime_loss(self, encoder_kv, prime_t):
        if self.use_tokens:
            ...
            prime_loss = t.tensor(0.0, device='mps')
        return prime_loss
